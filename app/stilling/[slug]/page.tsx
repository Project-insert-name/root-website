...
const JobAdvertPage: AsyncComponent<{ params: Params; }> = async ({ params }) => {
    ...
    return (
        <div className={ "container sm:w-[1000px] px-2 mx-auto" }>
            ...
            <h3 className={ "text-center sm:text-2xl text-lg" }>
                { ad.company }
            </h3>
            { ad.image &&
                <SanityImage className={ "mx-auto" } image={ ad.image }
                             width={ 500 } height={ 500 } alt={ ad.image_alt ?? `Bilde for ${ ad.title }` } />
            }
...
const Deadline: Component<{ deadline: string } & ChildProps> = ({ deadline, children }) => {
    ...
    return <DateIcon title={ `Søknadsfrist ${ formatDate }` }>{ children } { formatDate }</DateIcon>
}
