 ...
 */
const HamburgerMenu: Component<{ sliderClassName?: string } & DefaultProps> = ({
    className,
    sliderClassName,
}) => {
    ...
    return (
        <>
            {isOpen && (
                <div
                    className={"absolute left-0 top-0 z-20 h-full w-screen bg-black/20"}
                    onClick={() => setIsOpen()}
                />
            )}
            ...
            <Slider
                ...
                } transition-all duration-200 ${sliderClassName}`}
            />
        </>
    )
}
...
const Slider: Component<{ onClick: VoidFunction } & DefaultProps> = ({ className, onClick }) => (
    <div className={`absolute top-0 z-50 h-fit w-1/2 rounded-l-xl bg-white p-2 ${className}`}>
        <button onClick={onClick} className={"flex w-full justify-end p-2"} title={"Lukk"}>
